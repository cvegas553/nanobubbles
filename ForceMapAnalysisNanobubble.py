...
Usage is simple: input the required variables in the variable declaration below 
...
You can analyse multiple images at once - just put multiple values in the 
square brackets for DataPath, Resolution and SideLength.
...
ExportDir = 'F:\Temp for export'  
            #path for the summaries to be saved
        
Sensitivity =  7.04865330170149E-08 #m/V

Resolution = [64]      #Resolution is the number of pixels in 
                       #the image (must be less than 1000)                 

SideLength = [4]           #scan size in micrometers
...
def main(DataPath, Sensitivity, Resolution, SideLength):
    ...
    for i in range(0, Resolution):
        for j in range(0, Resolution):
            ...
            ExtendY = ExtendY*Sensitivity #convert y axis to m
            RetractY = RetractY*Sensitivity
            ...
            if Plot or SavePlot or PlotThisOne:
              ...
              fig, ax = plt.subplots(figsize=(50*mm,(2/3)*50*mm))
              ...
              SavePathPlot = os.path.join(DataPath, 
                                          f'Line{GenNum(i)}Point{GenNum(j)}.png')
              
              if SavePlot:
                SaveFolder = os.path.join(ExportDir, 
                ...
                SavePathPlot = os.path.join(SaveFolder, 
                                            f'Line{GenNum(i)}Point{GenNum(j)}.png')
...
def WriteFile(path, x, y, z, Bottom, xLoc, yLoc, air_thick):          
